...
if ($object->xpdo) {
    switch ($options[xPDOTransport::PACKAGE_ACTION]) {
        ...
        case xPDOTransport::ACTION_INSTALL:
            $modx =& $object->xpdo;
            $modelPath = $modx->getOption('tagfaster.core_path',null,$modx->getOption('core_path').'components/tagfaster/').'model/';
            $modx->addPackage('tagfaster',$modelPath);

            $manager = $modx->getManager();
			$manager->createObjectContainer('Tags');
			$manager->createObjectContainer('DocTags');
            break;
    }
}
